...
class TestEditEventFormValidation(unittest.TestCase):
    def setUp(self):
        ...
        self.form = CreateEventForm()  # Utiliser le même formulaire pour l'édition
    ...
    def _prepare_base_form(self):
        ...
        self.form.title.data = 'Titre Test Édition'
        self.form.description.data = 'Description Test Édition'
        self.form.event_date.data = (datetime.now() + timedelta(days=30)).strftime('%d/%m/%Y')
        self.form.event_time.data = '14:30'
        self.form.location.data = 'Lieu Test Édition'

    def test_edit_optional_fields(self):
        """Teste les champs optionnels lors de l'édition"""
        # Tester la capacité
        capacity_test_cases = [
        ...
        for value, expected_validity, expected_error in capacity_test_cases:
            ...
            print(f"\nTesting capacity during edit: {value}")
            ...
            self.assertEqual(validation_result, expected_validity, 
                             f"Échec pour la capacité {value} lors de l'édition. Résultat attendu : {expected_validity}")
        ...
        for value, expected_validity, expected_error in price_test_cases:
            ...
            print(f"\nTesting price during edit: {value}")
            print(f"Form data: {self.form.data}")
            print(f"Validation result: {validation_result}")
            print(f"Form errors: {self.form.errors}")
            ...
            self.assertEqual(validation_result, expected_validity, 
                             f"Échec pour le prix {value} lors de l'édition. Résultat attendu : {expected_validity}")
